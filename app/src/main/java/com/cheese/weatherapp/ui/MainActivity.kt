...
import com.example.mikmok.data.models.Weathers
        ...
        client.newCall(request).enqueue(object : Callback {
            ...
            override fun onResponse(call: Call, response: Response) {
                response.body?.string()?.let { jsonString ->
                    val result = Gson().fromJson(jsonString, Weathers::class.java)
                    runOnUiThread {
                        if (result.cod.toString() == Constants.NOT_FOUND) {
                            ...
                            binding.run {
                                ...
                                binding.description.text =
                                    result.weather[Constants.INDEXT_WEATHER].description
                                binding.maxMin.text =
                            ...
                            when (result.weather[Constants.INDEXT_WEATHER].main.uppercase()) {
                                ...
                                WeatherState.CLEAR.toString() -> {
                                    ...
                                    binding.weather.loop(true)
                                }
                                WeatherState.RAIN.toString() -> {
                                    binding.weather.setAnimation(R.raw.cloud)
                                    binding.weather.loop(true)
                                }
                            }
                        }
